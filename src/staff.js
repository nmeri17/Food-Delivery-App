...
$(document).ready(function () {
	...
	$("#confirm-assign a").click(function (e) {
		...
		assigned.each(function() {
			$(this).html(`<a href=delivered/?customer=${encodeURIComponent($(this).siblings().eq(1).text().trim())}&ID=${$(this).siblings().eq(4).text().trim()}>${assigned.text()}</a>`)
		})
		.children().addClass('delivered');
		
		$('.nobody').click(deliveredClosure($('.nobody')).bind($('.nobody')));
// se fin
})